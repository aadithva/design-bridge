...
const webDist = path.join(__dirname, '../../client/dist');
...
async function createStorageProviders(): Promise<{
  ...
  if (STORAGE_BACKEND === 'file') {
    const { FileStorageProvider } = await import('./storage/file-provider.js');
    return {
      analysisStore: new FileStorageProvider<AnalysisResult>(STORAGE_DIR, 'analyses'),
      reviewStore: new FileStorageProvider<ReviewResult>(STORAGE_DIR, 'reviews'),
    };
  }
  return {
...
async function boot() {
  ...
  app.post('/api/discover/match-files-bulk', async (req, res) => {
    try {
      ...
      console.log(`[bulk-match] ${(files as any[]).length} Figma files × ${craftPRs.length} craft PRs`);
      ...
      for (const f of files as Array<{ name: string; fileKey?: string }>) {
        ...
        if (matches.length > 0) {
          const top = matches[0];
          console.log(`[bulk-match]   "${f.name}" → ${matches.length} matches, top: PR#${top.pullRequestId} score=${top.score.toFixed(3)} reason=${top.matchReason}`);
        }
      }
  ...
  app.post('/api/discover/warm-craft-prs', async (req, res) => {
    try {
      ...
      craftPRCache.set(cacheKey, { prs: craftPRs, timestamp: Date.now() });
      const withUiFiles = craftPRs.filter((p: any) => p.uiFiles?.length > 0).length;
      console.log(`[warm-craft-prs] ${craftPRs.length} total PRs, ${withUiFiles} with UI files`);
  ...
  const server = app.listen(PORT, () => {
    console.log(`Prism API running at http://localhost:${PORT}`);
  });
