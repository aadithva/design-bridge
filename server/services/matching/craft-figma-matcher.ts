...
export function fuzzyScore(a: string, b: string): number {
  ...
  // Containment check: if one string fully contains the other, high score
  const aStripped = a.replace(/\s+/g, '');
  const bStripped = b.replace(/\s+/g, '');
  if (aStripped.length >= 2 && bStripped.length >= 2) {
    if (aStripped.includes(bStripped) || bStripped.includes(aStripped)) {
      // Scale by length ratio so near-exact matches score higher
      const ratio = Math.min(aStripped.length, bStripped.length) / Math.max(aStripped.length, bStripped.length);
      const containmentScore = 0.6 + 0.4 * ratio; // 0.6–1.0
      // Still compute word overlap and take the max
      const wordScore = wordOverlap(a, b);
      return Math.max(containmentScore, wordScore);
    }
  }
...
/** Word-overlap similarity (Dice coefficient) */
...
function extractBranchKeywords(sourceRefName: string): string {
  return sourceRefName
    .replace(/^refs\/heads\//, '')
    .split(/[\/\-_]/)
    .filter(w => w.length > 1 && !BRANCH_NOISE_WORDS.has(w.toLowerCase()))
    .join(' ')
    .toLowerCase();
}
...
export function matchFigmaFileToPRs(
  ...
  for (const pr of craftPRs) {
    ...
    for (const file of pr.uiFiles) {
      const { segments, compound } = extractFolderNames(file);
      // Check compound path first (higher quality match — apply 1.2x boost, capped at 1.0)
      if (compound && segments.length > 1) {
        const normCompound = normalize(compound);
        const rawScore = fuzzyScore(normFileName, normCompound);
        const score = Math.min(rawScore * 1.2, 1.0);
